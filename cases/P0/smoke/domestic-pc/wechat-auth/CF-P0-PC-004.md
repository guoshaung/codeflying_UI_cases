...
- 问题类型：产品体验
- 用户类型：免费用户 / 付费用户
- 前置条件：未登录；具备微信测试账号和人工扫码条件。
- Agent 分组：auth_identity_runner
- 账号类型：anonymous + manual-wechat
- 是否修改数据：no
- 数据锁：`account:{phone}`
- 清理动作：清理动态测试账号及登录会话
...
2. 进入 FM 路径：`国内主站 / 登录 / 微信登录`。
3. 执行原始测试点描述的操作：用户微信扫码登录成功。
4. 记录页面状态、URL、关键 DOM、接口结果和截图。

## 预期结果

1. 原始测试点描述的目标结果正确发生：用户微信扫码登录成功。
