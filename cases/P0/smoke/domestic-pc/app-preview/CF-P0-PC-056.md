# CF-P0-PC-056 会员用户点击“好呀”开启全天候预览
...
4. 若有“启动预览”先点击并等待加载完成；点击可见且文字精确为“好呀”的 `button.upgrade-button`。
5. 点击可见的“进入在线模式/全天候在线”，等待接口业务成功和页面进入全天候状态。
6. 若进入时已经是全天候状态，保留状态证据并按幂等成功处理。
4. 记录页面状态、URL、关键 DOM、接口结果和截图。

## 预期结果

1. 原始测试点描述的目标结果正确发生：会员用户点击“好呀”开启全天候预览。
