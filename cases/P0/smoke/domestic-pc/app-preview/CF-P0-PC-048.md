# CF-P0-PC-048 点击管理后台可跳转至应用管理后台页面
...
4. 点击可见的“管理后台/Management/Admin”入口，以目标 URL、标题或管理后台业务 DOM 确认成功。
4. 记录页面状态、URL、关键 DOM、接口结果和截图。

## 预期结果

1. 原始测试点描述的目标结果正确发生：点击管理后台可跳转至应用管理后台页面。
2. 页面无白屏、无未处理报错，相关功能可继续操作。

## 通过标准

- 管理后台目标页面或面板真实可见；不能只以点击成功判 PASS。
- 禁止在 `/my-apps` 列表页搜索“管理后台”并据此写 FAIL。
