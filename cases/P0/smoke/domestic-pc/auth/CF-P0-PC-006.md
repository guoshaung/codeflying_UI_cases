# CF-P0-PC-006 登录成功后进入应用创建页面 codeflying.net/develop
...
2. 进入 FM 路径：`国内主站 / 登录`。
3. 执行原始测试点描述的操作：登录成功后进入应用创建页面 codeflying.net/develop。
4. 记录页面状态、URL、关键 DOM、接口结果和截图。

## 预期结果

1. 原始测试点描述的目标结果正确发生：登录成功后进入应用创建页面 codeflying.net/develop。
